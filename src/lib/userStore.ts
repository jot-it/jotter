...
import { rootProvider, useAwareness } from "./collaboration";
...
export function useOthers() {
  ...
  awareness.forEach((state, clientId) => {
    if (clientId === rootProvider.awareness.clientID) return;
    if (state.user) {
...
function setAwarenessUser(user: User) {
  rootProvider.awareness.setLocalStateField("user", user);
}
